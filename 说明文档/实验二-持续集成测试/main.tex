...
不是最终课程报告。文中明确区分本地复演与 GitHub 托管运行，并用真实运行号、JUnit 和制品记录支持云端结论。
}}
\end{minipage}
\end{center}
...
云端运行结果：& GitHub Actions 运行 32258964845，结论 success \\
...
伪造云端结果 & 报告结论不可复核 & 远程未运行时写“待验证”；运行后记录 run ID、提交号、JUnit 与制品 \\
\bottomrule
\end{longtable}

最后一项尤其重要。项目最初只完成本地复演，因此当时只能写“待验证”；随后接入公开仓库并成功执行 GitHub Actions 运行 32258964845，文档才把云端状态更新为 success。这体现结论随证据演进，而不是事后把本地结果改名。
...
Local verdict: passed; cloud status is recorded separately
\end{lstlisting}
...
39 项包括 REST 客户端的输入与契约测试、7 项真实接口测试、风险规则参数化测试、性能工作负载等价性测试以及 CI 配置自检。该本地数字本身不证明 Ubuntu Runner 已执行；云端结论必须由后续 run ID、作业步骤、JUnit 和 artifact 单独支持。

\section{真实云端运行与制品核验}

项目推送到公开仓库 \url{https://github.com/diydashi/campus-weather-lab} 后，\code{push} 事件触发真实工作流。关键身份如下：

\begin{lstlisting}[style=shellstyle]
commit: a36b2f445dbf9dd7207100029c6e4d9af0cfb7cf
run id: 32258964845
job id: 96087323031
created: 2026-08-19 21:35:31 +08:00
completed: 2026-08-19 21:36:00 +08:00
...
\begin{table}[htbp]
\centering
\caption{GitHub Actions JUnit 核验结果}
\begin{tabular}{lrrrrr}
\toprule
测试层 & tests & failures & errors & skipped & time \\
\midrule
离线测试 & 32 & 0 & 0 & 0 & 0.148 s \\
真实接口测试 & 7 & 0 & 0 & 0 & 4.725 s \\
\bottomrule
\end{tabular}
\end{table}

作业的检出源码、配置 Python、安装依赖、两层 pytest、构建、上传报告和上传分发包步骤全部为 success。构建日志出现“Successfully built”，下载的 \code{python-distributions} 中确有 wheel 与 sdist；\code{pytest-reports} 中两份 JUnit XML 已解析为上表数据。两个 artifact 分别为 1,479 B 和 38,182 B 的 ZIP，并设置在 2026年11月17日过期。

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.97\textwidth,height=0.42\textheight,keepaspectratio]{../../课程报告/images/github-actions-run.png}
  \caption{GitHub Actions 运行 32258964845 的成功作业与制品}
  \label{fig:github-actions-run}
\end{figure}
\clearpage

图~\ref{fig:github-actions-run} 同时显示 Success 状态、绿色 \code{test-and-build} 作业和两个 artifact。图中的黄色 Annotations 是 GitHub 对相关 Action 所用 Node.js 版本迁移的提醒，不是测试失败，也不改变本次作业的 success 结论。运行页为 \url{https://github.com/diydashi/campus-weather-lab/actions/runs/32258964845}；机器可读摘要另存于 \path{reports/experiment-2/github-run-32258964845/}。GitHub 官方 Python CI 教程给出了使用 \code{setup-python}、pytest、构建和制品上传的典型方式\cite{github-python-ci}。
...
本实验建立了从源码到可复核证据的最小持续集成链：事件触发、准备 Python、安装依赖、分层执行 pytest、生成 JUnit、构建 wheel 与 sdist、上传报告和分发包。2026年8月19日的本地复演中 39 项测试全部通过；随后真实 GitHub Actions 运行 32258964845 再次完成 32 项离线测试和 7 项接口测试，JUnit 均为零失败、零错误，两个分发包和两类 artifact 也核验成功。实验二所要求的代码提交到后台测试、报告与自动打包流程已经完整闭环。
